...
def seqProbes(mb_seq, mb_size, mb_sscount, probe):
    ...
    Tml = []
    for i in basesl:
        Tmx = mt.Tm_NN(i, dnac1 = 50000, dnac2 = 50000, Na = 100, nn_table = mt.R_DNA_NN1, saltcorr = 1)
        Tml.append(int(Tmx))
...
if __name__ == "__main__":
    ...
    subprocess.check_output(["oligoscreen", mb_userpath+'/oligoscreen_input.lis', mb_userpath+'/oligoscreen_output.csv', '--DNA'])
    ...
    for x in range(1, int(i)+1):
        subprocess.check_output(["fold", mb_userpath+"/Seq"+str(x)+".seq" , mb_userpath+"/Seq"+str(x)+ ".ct", "--DNA"])
